adj = {1: [(7, 459), (8, 696), (2, 117), (16, 406), (5, 281), (3, 1318)],
...
# 目标城市列表（例如：上海，广州，武汉）
target_cities = ["天津", "上海", "重庆", "石家庄", "太原", "呼和浩特", "沈阳", "长春", "哈尔滨",
    "北京", "杭州", "合肥", "福州", "南昌", "济南", "郑州", "武汉", "长沙", "广州", "南宁",
    "海口", "成都", "贵阳", "昆明", "拉萨", "西安", "兰州", "西宁", "银川", "乌鲁木齐", "香港", "澳门"
]
target_nodes = [cities.index(city) + 1 for city in target_cities]
...
for target in target_nodes:
    ...
    distance = nx.dijkstra_path_length(G, source=source_node, target=target, weight='weight')
...
for target in target_nodes:
    ...
    for u, v in path_edges:
        all_path_edges.append((u, v))
        # 存储边的权重信息
        weight = G[u][v]['weight']
        edge_weights[(u, v)] = weight
        edge_weights[(v, u)] = weight  # 无向图需要双向存储

# 去除重复的边
unique_path_edges = list(set(all_path_edges))

# 绘制高亮的最短路径边
nx.draw_networkx_edges(G, pos, 
                      edgelist=unique_path_edges, 
                      edge_color="red", 
                      width=2)

# 只为高亮的边添加权重标签
edge_labels = {(u, v): edge_weights[(u, v)] for u, v in unique_path_edges}
nx.draw_networkx_edge_labels(G, pos, edge_labels=edge_labels, font_family="SimHei", font_size=8)

# 在控制台打印路径长度信息
print("\n从南京到各目标城市的最短路径长度：")
print("-" * 50)
print(f"{'目标城市':<10} {'路径长度(km)':<15} {'路径':<20}")
print("-" * 50)
...
# 输出路径信息
for target in target_nodes:
    city_name = cities[target-1]
    distance = shortest_distances[target]
    path_names = [cities[node-1] for node in shortest_paths[target]]
    path_str = " → ".join(path_names)
    print(f"{city_name:<10} {distance:<15.2f} {path_str}")

print("-" * 50)
print(f"所有最短路径总长度: {total_distance} km")

# 设置标题
plt.title("南京到各主要城市的最短路径网络", fontsize=16)
