...
nx.draw_networkx_edges(G, pos, 
                      edgelist=[(u, v) for u, v, d in mst_edges], 
                      edge_color="red", 
                      width=2)

# 绘制边的权重标签
edge_labels = {(u, v): d['weight'] for u, v, d in mst_edges}
nx.draw_networkx_edge_labels(G, pos, edge_labels=edge_labels, font_family="SimHei", font_size=8)

# 在控制台打印最小生成树信息
print("\n最小生成树的边:")
print("-" * 50)
print(f"{'起点':<10} {'终点':<10} {'距离(km)':<15}")
print("-" * 50)

for u, v, d in sorted(mst_edges):
    city_u = cities[u-1]
    city_v = cities[v-1]
    print(f"{city_u:<10} {city_v:<10} {d['weight']:<15}")

print("-" * 50)
print(f"最小生成树总长度: {total_weight} km")

# 设置标题
plt.title("中国主要城市最小生成树", fontsize=16)
